...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        n = len(y)
        loss = -np.sum(y * np.log(y_pred_proba) + (1 - y) * np.log(1 - y_pred_proba)) / n
        ...
        x1_values = np.linspace(x_min, x_max, 200)
        x2_values = -intercept - slope*x1_values
        plt.plot(x1_values, x2_values, label='Decision Boundary', color='black', linewidth=2, linestyle='--')
